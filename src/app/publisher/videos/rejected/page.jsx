...
export default function PublisherRejectedVideosPage() {
  ...
  const checkPublisherAccess = async () => {
    try {
      ...
      if (!token) {
        router.push('/login?redirect=/publisher/videos/rejected');
        return;
      }
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/publisher/videos?status=rejected`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch rejected videos');
      }
    ...
    <div className="space-y-8">
      ...
      {!loading && !error && videos.length === 0 && (
        <div className="bg-white shadow rounded-lg p-8 text-center">
          <p className="text-gray-500">No rejected videos</p>
        </div>
      )}
