...
export default function PublisherApprovedVideosPage() {
  ...
  const checkPublisherAccess = async () => {
    try {
      ...
      if (!token) {
        router.push('/login?redirect=/publisher/videos/approved');
        return;
      }
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/publisher/videos?status=approved`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  <div className="text-xs text-slate-500">
                    {new Date(video.published_on || video.created_on).toLocaleDateString('en-US', { 
                      month: 'short', 
                      day: 'numeric',
                      year: 'numeric'
                    })}
