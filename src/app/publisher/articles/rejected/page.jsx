...
export default function PublisherRejectedArticlesPage() {
  ...
  const checkPublisherAccess = async () => {
    try {
      ...
      if (!token) {
        router.push('/login?redirect=/publisher/articles/rejected');
        return;
      }
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/publisher/articles?status=rejected`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch rejected articles');
      }
    ...
    <div className="space-y-8">
      ...
      {!loading && !error && articles.length === 0 && (
        <div className="bg-white shadow-sm rounded-lg p-12 text-center">
          <p className="text-slate-600">No rejected articles</p>
        </div>
      )}
