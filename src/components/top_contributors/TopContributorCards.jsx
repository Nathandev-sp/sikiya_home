...
/**
 * Profile cards for /top_contributors — same layout as the live section (avatars, verified badge, bio, tags).
 * Wire this back into `src/app/top_contributors/page.jsx` when journalist recruitment / featured list is ready.
 */
...
export function TopContributorCards({ profiles = [], verifiedProfileLabel }) {
  return (
    <ul className="mx-auto mt-14 grid max-w-6xl list-none grid-cols-1 gap-8 sm:grid-cols-2 lg:mt-16 lg:grid-cols-4 lg:gap-6">
      {profiles.map((person, idx) => {
        const key = `${idx + 1}`
        const remote =
          typeof person.profileImageUrl === 'string' && person.profileImageUrl.trim().startsWith('http')
            ...
            <article className="group flex h-full flex-col rounded-2xl border border-transparent bg-white p-6 pb-5 shadow-[0_12px_40px_-18px_rgba(24,16,10,0.18)] transition-[border-color,box-shadow,transform] duration-200 ease-out hover:-translate-y-0.5 hover:border-[#a88366]/90 hover:shadow-[0_20px_48px_-20px_rgba(24,16,10,0.22)] focus-within:border-[#a88366]/90">
              <div className="relative mx-auto" style={{ width: AVATAR_PX, height: AVATAR_PX }}>
                ...
                <VerifiedShieldBadge
                  ...
                  ariaLabel={verifiedProfileLabel}
                />
              </div>

              <div className="mt-4 text-center">
                <h2 className="font-hero text-lg font-bold tracking-tight text-[#1a1814] sm:text-xl">{person.name}</h2>
              ...
              <div className="mt-5 border-t border-[#efe9e3] pt-5">
                <p title={person.bio} className="font-sans text-sm leading-relaxed text-[#534c45] line-clamp-2">
                  {person.bio}
                </p>
...
function VerifiedShieldBadge({ className, ariaLabel }) {
  return (
    <span className={['inline-flex shrink-0', className].filter(Boolean).join(' ')} role="img" aria-label={ariaLabel}>
  ...
  return (
    ...
    >
      <path
        ...
        d="M10 17.5s-5.75-5.925-5.75-9.75A5.75 5.75 0 0110 2a5.75 5.75 0 015.75 5.75c0 3.825-5.75 9.75-5.75 9.75Z"
      />
